...
You MUST update agent instructions to require ./harness usage.
...
- .harness/README.md
- agent instructions requiring use of ./harness
>>
...
RUN `verify-harness`
RETURN: format="HARNESS_RESULT", command_summary=COMMAND_MAP, evidence_summary=EVIDENCE_FILES, friction_summary=FRICTION_ENTRIES, harness_path=HARNESS_PATH, verdict=VERIFY_VERDICT, verify_summary=VERIFY_OUTPUT
</process>
...
TRY:
  ...
  USE `edit/editFiles` where: filePath=INSTRUCTIONS_PATH
RECOVER (err):
  USE `edit/createFile` where: content=INSTRUCTION_TEXT, filePath=INSTRUCTIONS_PATH
</process>
